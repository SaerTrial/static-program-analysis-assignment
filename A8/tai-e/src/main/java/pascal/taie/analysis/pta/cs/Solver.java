...
public class Solver {
    ...
    public void addWorkListEntry(CSVar csVar, CSObj transferObj){
        workList.addEntry(csVar, PointsToSetFactory.make(transferObj));
    }
    ...
    private void addReachable(CSMethod csMethod) {
        ...
        if (!callGraph.contains(csMethod)) {
            ...
            // TODO: if we set a global variable, then it would fail in OJ system with 800 out 813 stmts being successfully analyzed.
    ...
    private class StmtProcessor implements StmtVisitor<Void> {
        ...
        public Void visit(Invoke invoke) {
            ...
            // add a sink call into set
            if (taintAnalysis.isSinkCall(m))
                taintAnalysis.addSink(csManager.getCSCallSite(context, invoke), m);

            if (invoke.getResult() != null) {
                // taint generation
                CSObj taintObj = taintAnalysis.getTaintObj(invoke, invoke.getResult().getType());
                if (taintObj != null)
                    workList.addEntry(csManager.getCSVar(context, invoke.getResult()),
                            PointsToSetFactory.make(taintObj));

                // taint transfer
                taintAnalysis.aggregateArgToResult(csManager.getCSCallSite(context, invoke), invoke.getResult().getType());
//                int idxArgToResult = taintAnalysis.isArgToResult(invoke, invoke.getResult().getType());
//                if (idxArgToResult != -1) {
//                    for (int idx = 0; idx < m.getIR().getParams().size(); idx++) {
//                        if (idx != idxArgToResult) continue;
//                        csManager.getCSVar(context, invoke.getInvokeExp().getArgs().get(idx)).getPointsToSet().forEach(csObj -> {
//                            if (taintAnalysis.isTaintObj(csObj)) {
//                                CSObj transfer = taintAnalysis.transferTaintObj(csObj, invoke.getResult().getType());
//                                if (transfer != null)
//                                    // context comes from caller
//                                    workList.addEntry(csManager.getCSVar(context, invoke.getResult()),
//                                            PointsToSetFactory.make(transfer));
//                            }
//                        });
//                    }
//                }
    ...
    // deal with taint transfer in an out-of-order manner
    private void processTransfer(CSObj csObj) {
        // arg->base
        // arg->result
        if (taintAnalysis.isTaintObj(csObj)) taintAnalysis.transfer();
    }
    ...
    private void processCall(CSVar recv, CSObj recvObj) {
        ...
        for (Invoke invoke : recv.getVar().getInvokes()){
            ...
            // add a sink call into set
            if (taintAnalysis.isSinkCall(m))
                taintAnalysis.addSink(csManager.getCSCallSite(recv.getContext(), invoke), m);

            if (invoke.getResult() != null) {
                // taint generation
                CSObj taintObj = taintAnalysis.getTaintObj(invoke, invoke.getResult().getType());
                if (taintObj != null)
                    workList.addEntry(csManager.getCSVar(recv.getContext(), invoke.getResult()),
                            PointsToSetFactory.make(taintObj));

                // taint transfer
                if (taintAnalysis.isBaseToResult(invoke, invoke.getResult().getType())) {
                ...
                taintAnalysis.aggregateArgToResult(csManager.getCSCallSite(recv.getContext(), invoke), invoke.getResult().getType());
//                int idxArgToResult = taintAnalysis.isArgToResult(invoke, invoke.getResult().getType());
//                if (idxArgToResult != -1) {
//                    for (int idx = 0; idx < m.getIR().getParams().size(); idx++) {
//                        if (idx != idxArgToResult) continue;
//                        csManager.getCSVar(recv.getContext(), invoke.getInvokeExp().getArgs().get(idx)).getPointsToSet().forEach(csObj -> {
//                            if (taintAnalysis.isTaintObj(csObj)) {
//                                CSObj transfer = taintAnalysis.transferTaintObj(csObj, invoke.getResult().getType());
//                                if (transfer != null)
//                                    workList.addEntry(csManager.getCSVar(recv.getContext(), invoke.getResult()),
...
//                }
            ...
            // TODO: Out-of-order will affect taintTransfer? Yes, ignoring this may miss some stmts
            taintAnalysis.aggregateArgToBase(csManager.getCSCallSite(recv.getContext(), invoke), recv.getType(), recv);
//            int idxArgToBase = taintAnalysis.isArgToBase(invoke, recv.getType());
//            if (idxArgToBase != -1) {
//                for (int idx = 0; idx < m.getIR().getParams().size(); idx++) {
//                    if (idx != idxArgToBase) continue;
//                    for (CSObj csObj : csManager.getCSVar(recv.getContext(), invoke.getInvokeExp().getArgs().get(idx)).getPointsToSet()) {
//                        if (taintAnalysis.isTaintObj(csObj)) {
//                            CSObj transfer = taintAnalysis.transferTaintObj(csObj, recv.getType());
//                            if (transfer != null)
//                                workList.addEntry(recv, PointsToSetFactory.make(transfer));
//                        }
//                    }
//                }
//            }
